...
public class PlayerController : MonoBehaviour
{
    ...
    public float zRange1 = 15;
    public float zRange2 = 0;
    ...
    void Update()
    {
        ...
        //Limits how far the player can move in the negative x range
        if (transform.position.x < -xRange)
        {
            transform.position = new Vector3(-xRange, transform.position.y, transform.position.z);
        }
        //Limits how far the player can move in the positive x range
        if (transform.position.x > xRange)
        {
            transform.position = new Vector3(xRange, transform.position.y, transform.position.z);
        }
        //Limits how far the player can move in the negative z range
        if (transform.position.z < -zRange2)
        {
            transform.position = new Vector3(transform.position.x, transform.position.y, -zRange2);
        }
        //Limits how far the player can move in the positive z range
